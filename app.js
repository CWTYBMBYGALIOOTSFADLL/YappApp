...
// 🟢 Cache to store loaded messages per channel/DM so we don't re-read them
...
let isInputEmojiTarget = false; // Tracks if we are inserting into the text box or reacting to a message
...
// 🟢 FIXED: Initialize as an empty ARRAY [] instead of an object {}, and unified localStorage keys
...
// 🟢 FIXED: Unified variable name to 'showShorts' to match your rendering and save listeners
...
// 🟢 Auto-expand text area height as content grows
messageInput.addEventListener('input', function() {
  this.style.height = '50px'; // Reset to default base height
  const newHeight = Math.min(this.scrollHeight, 150); // Bind to max-height set in CSS
  this.style.height = `${newHeight}px`;
});

// 🟢 Submit form on 'Enter' key safely by mimicking a button click
messageInput.addEventListener('keydown', function(e) {
  if (e.key === 'Enter' && !e.shiftKey) {
    e.preventDefault(); // Stop standard line break
    const sendBtn = document.getElementById('send-btn');
    if (sendBtn) sendBtn.click(); // Mimics a physical button click safely!
  }
});
...
function applyShortsVisibility() {
  ...
  // 🟢 FIXED: Uses unified 'showShorts' variable
  if (showShorts) {
...
emojiPicker.addEventListener('emoji-click', async (event) => {
  ...
  // 🟢 CASE A: Insert emoji directly into the chat input bar
  if (isInputEmojiTarget) {
    messageInput.value += selectedEmoji;
    emojiPickerWrapper.style.display = 'none';
    isInputEmojiTarget = false; // Reset state
    
    // Trigger height auto-resize if the line wrapped
    messageInput.dispatchEvent(new Event('input'));
    messageInput.focus();
    return;
  }

  // CASE B: Standard message reaction
...
// 🟢 ADDED: Missing helper function to verify if a username is already registered
...
function enterChatApp(photoURL = "") {
  ...
  // 🟢 Detect if running inside Electron or a standard web browser
  const isElectron = navigator.userAgent.toLowerCase().includes('electron');

  if (isElectron) {
    // If they are in the desktop app, completely remove the button from the DOM
    const downloadBtn = document.getElementById('desktop-download-btn');
    if (downloadBtn) {
      downloadBtn.remove();
    }
  } else {
    // If they are on the website, keep the click event working
    const downloadBtn = document.getElementById('desktop-download-btn');
    if (downloadBtn) {
        ...
        if (change.type === 'added') {
           ...
           if (msgTime > loginSessionTime && data.sender !== currentUser && data.type !== 'system') {
               if (currentChatTarget !== data.channel) {
                   ...
                   // 🟢 UPDATED: Only play sound if you aren't currently focused on the browser tab
        ...
        if (change.type === 'added') {
           ...
           if (msgTime > loginSessionTime && data.sender !== currentUser) {
               if (currentChatTarget !== data.sender) {
                   ...
                   // 🟢 UPDATED: Only play sound if DMs are muted, DND is off, and you're not focusing on the window/this DM target
...
// 🟢 OPTIMIZED & REAL-TIME: Loads static profile data once, listens for live status updates lightweightly
function loadUsersSidebar() {
  const q = query(collection(db, "users"), orderBy("username", "asc"));
  
  // 1. Fetch all users once to build the layout (Huge read savings!)
  getDocs(q).then((snapshot) => {
    ...
    snapshot.forEach((docSnap) => {
      ...
      // Store static info in the cache
      ...
      if (userId !== currentUser) {
        ...
        userEl.setAttribute('data-user-id', userId); // 🟢 Attach ID to element for targeted live updates
    ...
    // 2. 🟢 Setup a live snapshot listener that ONLY processes changed status fields
    ...
    unsubscribeSidebar = onSnapshot(q, (liveSnapshot) => {
      liveSnapshot.docChanges().forEach((change) => {
        if (change.type === "modified") {
          ...
          // Directly update the status dot color in the DOM (No full layout re-renders!)
...
function switchChat(type, target) {
  ...
  // Initialize cache bucket for this channel if it doesn't exist
  if (!chatMessagesCache[target]) {
    chatMessagesCache[target] = [];
  }

  // 🟢 STEP 1: Render cached messages immediately (Zero reads!)
  ...
  if (chatMessagesCache[target].length > 0) {
    ...
    chatMessagesCache[target].forEach(msg => {
      displayMessage(msg.data, msg.id, type === 'global' ? "global_messages" : "private_messages");
    });
  ...
  // 🟢 STEP 2: Only query Firestore for new messages or initial load
  let q;
  if (type === 'global') {
    q = query(
  ...
  unsubscribeChat = onSnapshot(q, (snapshot) => {
    ...
    // Update the cache with the fresh dataset
...
function displayMessage(data, docId, collectionName, prepend = false) {
  ...
  // 🟢 FIXED: Decrypt the quoted text correctly
  let quotedHtml = "";
  if (data.replyTo) {
    let replySnippet = "";
    if (data.replyTo.type === 'image') {
      ...
      // If the reply text is encrypted, decrypt it. If not, use it directly.
  ...
  addReactionBtn.addEventListener('click', (e) => {
    isInputEmojiTarget = false; // 🟢 Reset input flag so this is treated as a reaction
...
async function sendPayloadToDatabase(textContent, imageUrlContent, payloadType) {
  ...
  // 🟢 FIXED: Attach the reply object to the Firestore document if you are replying to someone
  if (replyingTo) {
    payload.replyTo = {
  ...
  try {
    ...
    // Clear reply state and close banner
    replyingTo = null; 
    replyBanner.style.display = 'none'; 
  }
}
...
messageForm.addEventListener('submit', async (e) => {
  e.preventDefault(); // 🔒 CRITICAL: This is what stops the page from refreshing!
  ...
  // Update the end of your messageForm submit listener:
  messageInput.value = '';
  messageInput.style.height = '50px'; // 🟢 Reset height visually back to 1 row
...
// 🟢 Handle Desktop App Download click
...
if (desktopDownloadBtn) {
  desktopDownloadBtn.addEventListener('click', () => {
    // Replace this link with the actual URL where your desktop app installer is hosted
    window.location.href = "https://store-eu-par-4.gofile.io/download/web/f7c1e65b-b4f4-4e04-9ce6-f0b79d10cfff/YappApp%20Desktop%20Installer.exe";
  });
}

// 🟢 Handle input bar emoji button click
const inputEmojiBtn = document.getElementById('input-emoji-btn');

if (inputEmojiBtn) {
  inputEmojiBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    isInputEmojiTarget = true; // Route emojis to the input bar
    targetReactionMessageId = null; // Clear active message reaction state

    const rect = inputEmojiBtn.getBoundingClientRect();
    emojiPickerWrapper.style.display = 'block';
    
    // Position the picker right above the new emoji button
